...
1. **Texto exacto**: el de este documento es una propuesta redactada en la sesión
   ...
   propuesta.
2. ✅ **Resuelto — disponibilidad del sprite** de Rotom-Ventilador (`10011`): no estaba
   embebido, se agregó (junto con `10009`, Rotom-Lavadora) al implementar
   `rotom-confirmation-dialogs.md` — ver el punto 1 resuelto ahí para el detalle completo
   (rutas exactas, y el `EXTRA_FORM_IDS` agregado a `scripts/download-sprites.py`).
